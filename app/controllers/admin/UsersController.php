...
class UsersController extends AdminController {
	...
	public function getIndex()
	{
		...
		$key = Input::get('key', '');
		$this->data['keyslug'] = $keyslug = Str::slug($key);
		$this->data['groupId'] = $groupId = Input::get('groupId', 0);
		$orderBy = Input::get('orderBy', 'create');
		...
		$users = $users
            ...
            ->where(function ($query) {
                if ($this->data['keyslug'])
                    $query->where('users.first_name', 'like', '%' . $this->data['keyslug'] . '%')
                		->orWhere('users.last_name', 'like', '%' . $this->data['keyslug'] . '%');
		...
		// Show the page
		return View::make('backend/users/index', compact('users', 'groups', 'key', 'groupId', 'orderBy', 'withTrashed', 'onlyTrashed'));
	}
	...
	{
		if ( !Permission::has_access('user', 'edit', $id) )
			return View::make('backend/notallow');
		...
		try
		{
			...
			// Get this user category permissions
		...
		// Show the page
		return View::make('backend/users/edit', compact('user', 'groups', 'userGroups', 'permissions', 'userPermissions', 'catPermissions'));
	}
	...
	public function postEdit($id = null)
	{
		...
		if ( ! $password = Input::get('password'))
		{
			...
			//$this->validationRules['password']         = 'required|between:2,32';
			//$this->validationRules['password_confirm'] = 'required|between:2,32|same:password';
		}
		...
		try
		{
			...
			$user->is_doctor   = Input::get('is_doctor', 0);
