...
@section('content')
	...
	@if (Session::has('youtubeVideoId'))
		Last upload...<br />
		<iframe width="100%" height="315" src="//www.youtube.com/embed/{{ Session::get('youtubeVideoId') }}?rel=0" frameborder="0" allowfullscreen></iframe>
	@endif

	@if (isset($authUrl))
		<a href="{{ $authUrl }}" target="_blank">Connect me</a>
	...
	@else
		{{ Form::open(array('url' => 'youtube-upload-example', 'class' => 'form', 'files' => true)) }}
			...
			<div class="form-group{{ $errors->has('description') ? ' has-error' : '' }}">
				...
				{{ Form::textarea('description', Input::old('description'), array('class' => 'form-control', 'placeholder' => 'description', 'style' => 'height: 80px;')) }}
			...
			<div class="form-group{{ $errors->has('status') ? ' has-error' : '' }}">
				...
				{{ Form::select('status', array('unlisted' => 'Unlisted', 'public' => 'Public', 'private' => 'Private'), Input::old('status'), array('class' => 'form-control')); }}
